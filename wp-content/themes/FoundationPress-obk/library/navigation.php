...
register_nav_menus(array(
	...
	'mobile-nav' => 'Mobile',
	'left-off-canvas' => 'left-off-canvas menu',
));
...
/**
 * Desktop navigation - left-off-canvas
 *
 * @link http://codex.wordpress.org/Function_Reference/wp_nav_menu
 */
if ( ! function_exists( 'foundationpress_left_off_canvas' ) ) {
	function foundationpress_left_off_canvas() {
		wp_nav_menu( array(
			'container'      => false,
			'menu_class'     => 'vertical accordion-menu menu',
			'items_wrap'     => '<ul id="%1$s" class="%2$s desktop-menu" data-accordion-menu>%3$s</ul>',
			'theme_location' => 'left-off-canvas',
			'depth'          => 3,
			'fallback_cb'    => false,
			'walker'         => new foundationpress_left_off_canvas_walker(),
		));
	}
}
